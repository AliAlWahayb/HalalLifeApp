...
let localConversations = new Map<string, Message[]>();
...
const NON_HALAL_INGREDIENTS = [
  "pork", "lard", "gelatin", "alcohol", "wine", "beer", "rum", "brandy", 
  "ethanol", "vanilla extract", "carmine", "cochineal", "rennet", "pepsin",
  "lipase", "blood", "bacon", "ham", "pancetta", "prosciutto", "sausage",
  "l-cysteine", "mono-glycerides", "di-glycerides", "e120", "e441", "e542"
];

// Common halal ingredients for reference
const HALAL_INGREDIENTS = [
  "vegetables", "fruits", "grains", "plant-based", "salt", "honey", "vegetable oil",
  "olive oil", "sugar", "rice", "milk", "eggs", "most seafood", "beef", "chicken", "lamb"
];

// Questionable ingredients that need verification
const VERIFICATION_NEEDED = [
  "glycerin", "glycerol", "emulsifiers", "enzymes", "flavorings", "whey", 
  "yogurt", "cheese", "shortening", "e471", "e472", "e473", "e475", 
  "mono and diglycerides", "lecithin", "magnesium stearate"
];

// A simple utility function to check if an ingredient is halal, haram, or unknown
function checkIngredientStatus(ingredient: string): { status: 'halal' | 'haram' | 'doubtful' | 'unknown', reason: string } {
  // Normalize the ingredient name
  const normalizedName = ingredient.toLowerCase().trim();
  
  // Common haram ingredients
  const haramIngredients = {
    'alcohol': 'Contains intoxicants which are prohibited in Islam',
    'ethanol': 'Form of alcohol which is prohibited in Islam',
    'ethyl alcohol': 'Form of alcohol which is prohibited in Islam',
    'wine': 'Contains alcohol which is prohibited in Islam',
    'beer': 'Contains alcohol which is prohibited in Islam',
    'rum': 'Contains alcohol which is prohibited in Islam',
    'pork': 'Derived from pig which is explicitly prohibited in Islam',
    'bacon': 'Derived from pig which is explicitly prohibited in Islam',
    'lard': 'Fat from pig which is prohibited in Islam',
    'gelatin': 'Often derived from non-halal animals unless specifically stated as halal or plant-based',
    'pepsin': 'Often derived from pig stomach unless specified otherwise',
    'rennet': 'Often derived from animal sources unless specified as microbial or vegetable rennet',
    'blood': 'Blood is explicitly prohibited in Islam',
    'l-cysteine': 'Often derived from human hair or duck feathers, requires halal certification',
    'carmine': 'Red colorant derived from insects, requires halal certification',
    'cochineal': 'Red colorant derived from insects, requires halal certification',
    'e120': 'Carmine, derived from insects, requires halal certification'
  };
  
  // Doubtful ingredients that need further verification
  const doubtfulIngredients = {
    'glycerin': 'Can be derived from animal or vegetable sources, needs halal certification',
    'glycerol': 'Can be derived from animal or vegetable sources, needs halal certification',
    'glycerine': 'Can be derived from animal or vegetable sources, needs halal certification',
    'emulsifier': 'Can contain animal derivatives, needs verification',
    'enzyme': 'Can be derived from microbial, plant, or animal sources, needs verification',
    'lipase': 'Can be derived from animal or microbial sources, needs verification',
    'mono and diglycerides': 'Can be derived from animal or vegetable sources, needs verification',
    'monoglycerides': 'Can be derived from animal or vegetable sources, needs verification',
    'diglycerides': 'Can be derived from animal or vegetable sources, needs verification',
    'e471': 'Mono and diglycerides, can be from animal or vegetable sources',
    'e472': 'Emulsifiers that can be from animal or vegetable sources',
    'lecithin': 'Usually plant-derived but can be from animal sources, check E number',
    'e322': 'Lecithin, usually plant-derived but verify source',
    'vanilla extract': 'May contain alcohol as a carrier, needs verification',
    'natural flavors': 'Can contain various compounds including animal derivatives',
    'natural flavoring': 'Can contain various compounds including animal derivatives',
    'flavoring': 'Can contain various compounds including animal derivatives',
    'whey': 'Dairy product that requires halal certification if enzyme-processed',
    'cheese': 'Often made with rennet, requires halal certification',
    'shortening': 'Can be from animal or vegetable sources, needs verification'
  };
  
  // Generally halal ingredients
  const halalIngredients = {
    'vegetable oil': 'Plant-based oil which is halal',
    'soy': 'Plant-based which is halal',
    'tofu': 'Plant-based which is halal',
    'rice': 'Plant-based which is halal',
    'wheat': 'Plant-based grain which is halal',
    'corn': 'Plant-based grain which is halal',
    'sugar': 'Plant-based sweetener which is halal',
    'salt': 'Mineral which is halal',
    'spices': 'Plant-based flavorings which are generally halal',
    'vegetable': 'Plant-based which is halal',
    'fruit': 'Plant-based which is halal',
    'water': 'Natural and halal',
    'honey': 'Natural sweetener which is halal',
    'vinegar': 'Fermented but not intoxicating, considered halal',
    'citric acid': 'Derived from citrus fruits, generally halal',
    'lactic acid': 'Usually derived from bacterial fermentation, generally halal',
    'vitamin': 'Generally halal if plant-derived or synthetic',
    'mineral': 'Generally halal',
    'xanthan gum': 'Derived from bacterial fermentation, generally halal',
    'guar gum': 'Derived from plants, generally halal',
    'pectin': 'Derived from fruit, generally halal',
    'baking powder': 'Chemical leavening agent, generally halal',
    'yeast': 'Micro-organism, generally halal',
    'artificial flavor': 'Synthetic flavoring, generally halal but verify',
    'color': 'Generally halal if synthetic, verify if natural',
    'preservative': 'Generally halal if synthetic, verify if natural'
  };
...
const callGoogleGenerativeAI = async (imageData: string, prompt: string): Promise<string> => {
  try {
    ...
    if (response.data && 
      ...
      console.log('Unexpected response format from Google API:', JSON.stringify(response.data).substring(0, 200) + '...');
...
const callDeepSeekAPI = async (message: string, conversationHistory: Message[] = []): Promise<string> => {
  try {
    ...
    if (isOffTopic) {
      userPrompt = "The user has asked about a non-halal food topic. Please politely redirect them to ask about halal food topics only.";
    } else if ((message.toLowerCase().includes("analyze") && message.toLowerCase().includes("product")) || 
...
const textOnlyFallback = async (prompt: string): Promise<string> => {
  try {
    ...
    const messages = [
      ...
      { 
        role: 'user', 
        content: "I couldn't analyze the image properly. For halal verification, I need to know the ingredients list. Remember to start response with VERDICT: UNKNOWN."
      }
    ];
...
const processApiResponse = (text: string): string => {
  ...
  if (processed.toLowerCase().includes("code") && processed.toLowerCase().includes("python") || 
      processed.toLowerCase().includes("javascript") || processed.toLowerCase().includes("function")) {
    processed = "I'm the Halal Assistant and can only help with halal food topics. For halal food questions, feel free to ask about ingredients, certifications, or dietary guidelines!";
  }
  
  // Fix duplicate VERDICT issue
  processed = processed.replace(/VERDICT:\s*(HALAL|HARAM|UNKNOWN)VERDICT:\s*(HALAL|HARAM|UNKNOWN)/i, 'VERDICT: $2');
  ...
  if (!processed.includes("VERDICT:")) {
    ...
    const isProductAnalysis = 
      ...
      (processed.toLowerCase().includes("halal status") && processed.toLowerCase().includes("analysis"));
      
    if (isProductAnalysis) {
      const lowerText = processed.toLowerCase();
      if (lowerText.includes('halal') && !lowerText.includes('haram') && !lowerText.includes('non-halal')) {
        processed = 'VERDICT: HALAL. ' + processed;
      } else if (lowerText.includes('haram') || lowerText.includes('non-halal') || lowerText.includes('forbidden')) {
...
export const ChatAPI = {
  ...
  sendMessage: async (request: ChatRequest, userId: string): Promise<ChatResponse> => {
    try {
      ...
      const hasExtractedIngredients = request.message.includes('Extracted ingredients:') || 
      ...
      if (request.image_data && request.image_data.startsWith('data:image')) {
        ...
        const ingredientMatch = request.message.match(/(?:is|check|halal status of)\s+([a-zA-Z0-9\s\-]+)\s+(?:halal|haram|permissible)/i);
